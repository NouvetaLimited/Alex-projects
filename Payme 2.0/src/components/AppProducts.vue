    ...
    <v-container fluid>
            ...
            <v-card>
                ...
                <v-data-table v-bind:headers="headers" v-bind:items="items" v-bind:search="search"
                    ...
                    <template  slot="items" slot-scope="props" class="body-2" >
                        <td class="body-2"  >{{ props.item.BrandName }}</td>
                        <td class="text-xs-left">{{ props.item.categoryName }}</td>
                        <td class="text-xs-left">{{ props.item.Product.product_name}}</td>
                        <td class="text-xs-left">{{ props.item.Product.description}}</td>
                        <td class="text-xs-left">KES {{ props.item.Product.price}}</td>
                        <td class="text-xs-left">{{ props.item.quantity}}</td>
                        <td class="text-xs-left"><img :src='getImageURL(props.item.Product.image)' height="90dp" width="90dp"/></td>
        ...
        <v-navigation-drawer right v-model="rightDrawer" fixed>
            <v-list>
                ...
                <v-list-tile>
                    <v-list-tile-title>Search Panel {{productSearch}}</v-list-tile-title>
                    <v-list-tile-action>
                ...
                <v-layout row>
                    <v-flex xs11 offset-xs1>
                        <v-text-field name="product" label="Product" light v-model="productSearch"></v-text-field>
                    </v-flex>
                </v-layout>
                <v-layout row>
                    <v-flex xs12>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            add () {
                this.$router.push('AppProductForm')
            },
            orderItems (Items) {
            ...
            getProducts () {
                ...
                data.append('TransactionType', 'getProduct')
                data.append('keyword', this.productSearch)
